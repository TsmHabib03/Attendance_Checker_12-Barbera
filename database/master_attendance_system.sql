...
BEGIN
    ...
    -- Get student class
    SELECT class INTO v_class FROM students WHERE lrn = p_lrn;
    
    IF v_class IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Student not found';
    END IF;
    
    SET v_scan_date = DATE(p_scan_time);
    SET v_scan_time = TIME(p_scan_time);
    ...
    FROM schedule 
    WHERE class = v_class 
      AND day_of_week = DAYNAME(v_scan_date)
      AND v_scan_time BETWEEN start_time AND end_time
      AND is_break = FALSE
    LIMIT 1;
    ...
    ELSE
        ...
        SELECT 
            CASE 
                WHEN TIMEDIFF(v_scan_time, start_time) <= '00:15:00' THEN 'present'
                ELSE 'late'
            END
        INTO v_status
        FROM schedule 
    ...
    VALUES (p_lrn, v_scan_date, v_scan_time, v_subject, v_period_number, v_status)
    ...
    SELECT v_status as status, v_subject as subject, v_period_number as period_number;
END //
